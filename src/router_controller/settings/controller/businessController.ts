...
// GET /businesses
export const getBusinesses = async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
// GET /businesses/:id
export const getBusinessById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
...
export const deleteBusiness = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
...
export const createOtherDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { country, currency, timeZone, salaryRelease, trackStaff } = req.body;

  try {
    const otherDetails = await prisma.otherDetails.create({
      data: { user: req.user.id, country, currency, timeZone, salaryRelease, trackStaff },
    });

    res.status(201).json(otherDetails);
  } catch (error) {
    res.status(500).json({ error: 'An error occurred while creating the other details.' });
  }
};

// Controller to update otherDetails
export const updateOtherDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
  const { country, currency, timeZone, salaryRelease, trackStaff } = req.body;
  ...
  try {
    ...
    res.status(500).json({ error: 'An error occurred while updating the other details.' });
  }
};

// Controller to delete otherDetails
export const deleteOtherDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;

  try {
    const deletedOtherDetails = await prisma.otherDetails.delete({ where: { id } });

    res.status(200).json(deletedOtherDetails);
  } catch (error) {
    res.status(500).json({ error: 'An error occurred while deleting the other details.' });
  }
};
export const getOtherDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'An error occurred while retrieving the other details.' });
    }
  };
  export const getOtherDetailsById = async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
